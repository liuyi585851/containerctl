...
def get_manifest(image,tag):
    url = f"https://registry-1.docker.io/v2/library/{image}/manifests/{get_digest(image,tag)}"
    headers = {
        'Authorization': f"Bearer {get_token(image)}",
...
def pull_image(image,tag,name):
    ...
    for layer in layers:
        ...
        os.makedirs(f"containers/{name}/images/blobs",exist_ok=True)
        download_file(url,headers,f"containers/{name}/images/blobs/layer-{count}.tar.gz")
        untar(f"containers/{name}/images/blobs/layer-{count}.tar.gz",f"containers/{name}/images/rootfs")
...
def operation_create():
    ...
    try:
        ...
        sys.exit()
    try:
        result=subprocess.run(['runc','spec','-b',f'containers/{name}/images'], capture_output=True,text=True)
        if result.returncode!=0:
        ...
        sys.exit()
    try:
        with open(f'containers/{name}/images/config.json','r') as f:
            config=json.load(f)
        
        config['process']['terminal']=False
        config['process']['args']=['sleep','1000000']
        with open(f'containers/{name}/images/config.json','w') as f:
            json.dump(config,f)
    except:
        print(f"发生未知错误")
        rollback(name)
        sys.exit()
    try:
        result=subprocess.run(['runc','create',f'--bundle=containers/{name}/images',f'{name}'])
        if result.returncode!=0:
...
def operation_start():
    ...
    try:
        ...
        result2=subprocess.run(['runc','create',f'--bundle=containers/{name}/images',f'{name}'])
